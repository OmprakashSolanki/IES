...
public class AdminServiceImpl implements AdminService {
	...
	public boolean registerAppAccount(AppAccount accModel) {
		...
		try {
			...
			String mailSub = appProperties.getProperties().get(AppConstants.ACCOUNT_REG_SUBJECT);
	...
	public String createMailBody(AppAccount model, String fileName) throws Exception {
			...
			while (line != null) {
				if (!line.equals("<br/>") && line.length() != 0) {
					...
					if (line.contains("APP_URL")) {
						line = line.replace("APP_URL", appProperties.getProperties().get(AppConstants.PROJECT_URL));
					}
		...
		try {
			...
			if (activeSw.equals("Y")) {
				try {
					...
					String emailSub = appProperties.getProperties().get(AppConstants.ACC_ACTIVATION_EMAIL_SUB);
					// get Activation file name
					String activationFileName = appProperties.getProperties()
							.get(AppConstants.ACCOUNT_ACTIVATE_EMAIL_FILE_NAME);
			...
			// send Account Activation Email
				...
				try {
					...
					String emailSub = appProperties.getProperties().get(AppConstants.ACC_DE_ACTIVATION_EMAIL_SUB);
					// get Activation file name
					String deActivationFileName = appProperties.getProperties()
							.get(AppConstants.ACCOUNT_DE_ACTIVATE_EMAIL_FILE_NAME);
	...
	/**
	 * This method is getting account from repository
	 * 
	 * @param accId
	 * @return AppAccount
	 */
	...
	public AppAccount findByAccountId(String accId) {
		logger.debug("AdminServiceImpl ::findByAccountId(); started");
		...
		model.setPassword(decryptedPwd);
		logger.debug("AdminServiceImpl ::findByAccountId(); ended");

		logger.info("findByAccountId() completed; ");
	...
	/**
	 * This method is used for checking login validation on the Basis of userName
	 * and password, checking Active Switch, checking roles
	 * 
	 * @param emailId, pwd
	 * @return String
	 */
	@Override
	public String appAccountLogin(String emailId, String pwd) {
		logger.debug("AdminServiceImpl::appAccountLogin(); method started");
		// encrypt the password
		String encryPwd = PasswordUtils.encrypt(pwd);

		// check for Email Availability
		AppAccountEntity entity = appAccRepository.findByEmail(emailId);
		if (entity != null) {
			if (encryPwd.equals(entity.getPassword())) {
				if (entity.getActiveSw().equals(AppConstants.ACTIVE_SW)) {

					if (entity.getRole().equals("Admin")) {
						logger.debug("AdminServiceImpl::appAccountLogin(); method ended");
						return "Admin";
					} else {
						logger.debug("AdminServiceImpl::appAccountLogin(); method ended");
						return "CaseWorker";
